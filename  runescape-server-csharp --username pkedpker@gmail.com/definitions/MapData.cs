...
namespace RunescapeServer.definitions
{
    class MapData
    {
         ...
         */
        public static MapList[] mapLists = new MapList[3306];
        ...
        public static void load()
        {
            ...
            //Dump mapdata to site.
            System.IO.StreamWriter file = new System.IO.StreamWriter(misc.getServerPath() + @"\data\mapdata\668dump.txt", true);
            ...
            for (int i = 0; i < 30000; i++)
            {
                ...
                try
                {
                    using (StreamReader sr = new StreamReader(misc.getServerPath() + @"\data\mapdata\" + i + ".txt"))
                    {
                        ...
                        mapDataDump = "http://www.rushgamer.com/mapdata/?version=668&region=" + i + "&keys=";
                        while ((line = sr.ReadLine()) != null)
                        {
                            if (!line.Equals(""))
                            {
                                mapDataDump += line.Trim() + "|";
                                list.data[regionId++] = int.Parse(line.Trim());
                            }
                        }

                        mapDataDump = mapDataDump.Substring(0, mapDataDump.Length - 1);
                        mapDataDump += "\");";
                        file.WriteLine(mapDataDump);
                        file.Flush();
                    }
                }
                catch (Exception)
