    ...
    class LocalEnvironment
    {
        ...
        public void updatePlayersInArea() //before update sent to all players.
        {
            ...
            foreach (Player p in Server.getPlayerList())
            {
                ...
                if (p == null || p == player || !p.isActive() || 
                    p.isDisconnected() || p.isDestroyed() || 
                    p.getUpdateFlags().isTeleporting() ||
                    !p.isVisible() || removedPlayers.Contains(p))
                    continue;
        ...
        {   
            ...
            foreach(Player p in seenPlayers) {
                ...
                if (!player.getLocation().withinDistance(p.getLocation()) || 
                    p.getUpdateFlags().isTeleporting() || p.isDisconnected() || 
                    p.isDestroyed() || !p.isVisible())
        ...
        public void updateNpcsInArea() //before update sent to all players.
        {
            ...
            foreach (Npc n in Server.getNpcList())
            {
                ...
                if (n == null || !n.isVisible() || removedNpcs.Contains(n))
                    continue;
        ...
        {   
            ...
            foreach(Npc n in seenNpcs) {
                ...
                if (!player.getLocation().withinDistance(n.getLocation()) || !n.isVisible())
                    removedNpcs.Add(n);
            }
        }
