...
namespace RunescapeServer.combat
{
    class MagicCombat : MagicData
    {
	    ...
	    public static void newMagicAttack(Player p, Entity target, int id, bool ancients) {
		    ...
		    double damage = CombatFormula.getMagicHit(p, target, getSpellMaxHit(p, index));
		    bool magicProtect = false;
		    if (target is Player) {
                magicProtect = ((Player)target).getPrayers().getHeadIcon() == PrayerData.PrayerHeadIcon.MAGIC;
		    }
		    if (magicProtect) {
			    damage *= 0.60;
		    }
            ...
            doMagicAttackEvent.setAction(() => {
				...
				if (p == null || p.isDead() || !fakeNPC && (target.isDead() || !target.isVisible() || target.isDestroyed())) {
					return;
				}
				...
				if (damage != 0) {
					Combat.checkRecoilRing(p, target, damage);
	    ...
	    public static void teleblockPlayer(Player killer, Player target) {
		    ...
		    if (target.getPrayers().getHeadIcon() == PrayerData.PrayerHeadIcon.MAGIC) {
	    ...
	    public static bool monsterInArea(Player p, Entity mainTarget) {
		    ...
		    foreach(Npc n in Server.getNpcList()) {
			    if (n == null || n.Equals(mainTarget) || n.isDead() || !n.isVisible() || n.isDestroyed()) {
				    continue;
			    }
		    ...
		    foreach(Player target in Server.getPlayerList()) {
			    if (mainTarget == null || target.Equals(mainTarget) || target.isDead() || !target.isVisible() || target.isDestroyed()) {
				    continue;
			    }
		    ...
		    if (mainTarget is Npc) {
			    foreach(Npc n in Server.getNpcList()) {
				    ...
				    if (n.getLocation().inArea(l.getX() - 1, l.getY() - 1, l.getX() + 1, l.getY() + 1)) {
					    ...
					    double damage = misc.randomDouble(getSpellMaxHit(p, index));
	    ...
	    public static bool freezeTarget(int spellToCast, Entity target) {
            if ((spellToCast != 34 && spellToCast != 36 && spellToCast != 40 && spellToCast != 19 && spellToCast != 23 && spellToCast != 27 && spellToCast != 31) || target.isFrozen())
            {
			    return false;
		    }
            int freezeDelay = getFreezeDelay(spellToCast);
		    target.setFrozen(true);
		    if (target is Player) {
			    ((Player)target).getPackets().sendMessage("You have been frozen!");
			    ((Player)target).getWalkingQueue().resetWalkingQueue();
		    }
            Event unFreezeEvent = new Event(freezeDelay);
	    ...
	    public static void addDamage(Entity killer, Entity target, double damage) {
		    if (!target.isDead()) {
