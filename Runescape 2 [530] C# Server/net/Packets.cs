...
namespace RunescapeServer.net
{
    class Packets
    {
        ...
        public void sendBankOptions()
        {
            ...
            sendClientScript(1451, new object[] { "" }, "");
        }
        ...
        public void animateInterface(int animationId, int interfaceId, int childId)
        {
        ...
        public void sendCloseInterface(int windowId, int interfaceId)
        {
            connection.SendPacket(new PacketBuilder().setId(149)
                .addUShort(count++)
                .addInt((windowId << 16) + interfaceId).toPacket());
        }
        ...
        public void configureGameScreen(int windowType)
        {
            ...
            if (windowType == 0 || windowType == 1)
            {
                ...
                //sendCloseInterface(548, 72); //works removing Energy bar
                //sendTab(73, 747); // Summoning bar (doesn't work)
                sendTab(8, 137); // Username on chat
            ...
            else if (windowType == 2 || windowType == 3)
            {
                ...
                sendTab(100, 662); // Summoning tab (this is a guess)
        ...
        public void displayInterface(int childId)
        {
            if (player.isHd())
            {
                sendInterface(0, 746, childId == 499 ? 5 : 6, childId);
                return;
            }
            sendInterface(0, 548, 11, childId);
        }
        ...
        public void sendTab(int interfaceId, int childId)
        {
            if (player.isHd())
            {
                sendInterface(1, childId == 137 ? 752 : 746, interfaceId, childId);
                return;
            }
            sendInterface(1, childId == 137 ? 752 : 548, interfaceId, childId);
        }
        ...
        public void sendOverlay(int childId)
        {
            if (player.isHd())
            {
                sendInterface(1, 746, 3, childId);
                return;
            }
            sendInterface(1, 548, 5, childId);
        }
